...
class Payment(models.Model):
    ...
    # Razorpay specific – add these back (critical for integration)
...
class WalletTransaction(models.Model):
    ...
    txn_type = models.CharField(max_length=20, choices=TRANSACTION_TYPE, default="CREDIT")  # rename to txn_type for clarity
    
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    
    balance_after = models.DecimalField(max_digits=10, decimal_places=2, null=True)
    
    reference_type = models.CharField(max_length=20, choices=REFERENCE_TYPE_CHOICES, blank=True)
    
    reference_id = models.CharField(max_length=50, blank=True)  # order_id or return_id
