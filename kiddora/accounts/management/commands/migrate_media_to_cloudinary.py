...
class Command(BaseCommand):
    help = (
        "Migrate existing media files to Cloudinary while preserving folder structure"
    )
        ...
        for root, dirs, files in os.walk(media_root):
            for file_name in files:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Uploaded: {relative_path} → {public_id} | URL: {secure_url}"
                        )
                    )
                    uploaded += 1
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"✗ Failed {relative_path}: {str(e)}")
                    )
                    errors += 1
