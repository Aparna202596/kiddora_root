...
def _logo_path():
    candidates = [
        ...
        os.path.join(settings.BASE_DIR, "static/images/kiddora_logo.png"),
        os.path.join(settings.STATICFILES_DIRS[0], "images/kiddora_logo.PNG") if getattr(settings, 'STATICFILES_DIRS', []) else None,
    ]
    for p in candidates:
...
def _build_pdf_header(elements, font_regular, font_bold, title_text, subtitle_text, start_date, end_date):
    ...
    # Report title
    elements.append(Paragraph(f'<font size="16" color="#1e293b"><b>{title_text}</b></font>', ParagraphStyle("title", fontName=font_bold, spaceAfter=4)))
    elements.append(Paragraph(f'<font size="10" color="#64748b">{subtitle_text}</font>', ParagraphStyle("sub", fontName=font_regular, spaceAfter=4)))

    # Period banner
    period_data = [[
        Paragraph(f'<font size="10" color="white"><b>Report Period: {start_date} → {end_date}</b></font>', ParagraphStyle("p", fontName=font_bold)),
        Paragraph(f'<font size="9" color="white">Downloaded on {now_str}</font>', ParagraphStyle("p2", fontName=font_regular, alignment=2)),
    ]]
...
def _kpi_table(rows_data, font_regular, font_bold, col_count=4):
    ...
    BG_COLORS = [
        ...
        "#f0fdf4", "#fee2e2", "#f3e8ff", "#dbeafe",
    ]
    TEXT_COLORS = [
        "#1e40af", "#166534", "#92400e", "#5b21b6",
        "#14532d", "#991b1b", "#6b21a8", "#1e40af",
    ]
    # Pad to full rows
    ...
    for i in range(0, len(rows_data), col_count):
        ...
        for j, (label, value) in enumerate(chunk):
            ...
            label_row.append(
                Paragraph(f'<font size="8" color="{TEXT_COLORS[idx]}">{label}</font>',
                          ParagraphStyle("kl", fontName=font_regular))
            )
            value_row.append(
                Paragraph(f'<font size="14" color="{TEXT_COLORS[idx]}"><b>{value}</b></font>',
        ...
        for j, (label, value) in enumerate(chunk):
            ...
            cell_row.append(
                Paragraph(
                    ...
                    ParagraphStyle("kc", fontName=font_regular, leading=18, spaceAfter=0)
                )
            )
    ...
    for row_i, row in enumerate(table_rows):
        for col_i in range(len(row)):
            ...
            style_cmds.append(('BACKGROUND', (col_i, row_i), (col_i, row_i), colors.HexColor(BG_COLORS[bg_idx])))
...
def _data_table(headers, rows, font_regular, font_bold, col_widths=None):
    ...
    header_cells = [
        Paragraph(f'<font size="9" color="white"><b>{h}</b></font>',
    ...
    for row in rows:
        table_data.append([
            Paragraph(f'<font size="8">{str(c)}</font>',
...
def admin_dashboard_view(request):
    ...
    orders_qs      = Order.objects.filter(order_date__date__gte=start_date, order_date__date__lte=end_date)
    paid_orders     = orders_qs.filter(payment_status='PAID')
    prev_orders_qs  = Order.objects.filter(order_date__date__gte=prev_start, order_date__date__lte=prev_end)
    ...
    new_customers    = customer_qs.filter(date_joined__date__gte=start_date, date_joined__date__lte=end_date).count()
    prev_customers   = customer_qs.filter(date_joined__date__gte=prev_start, date_joined__date__lte=prev_end).count()
    customer_growth  = growth_pct(new_customers, prev_customers)
    ordering_customers = orders_qs.values('user').distinct().count()
    repeat_customers   = orders_qs.values('user').annotate(c=Count('id')).filter(c__gt=1).count()

    payments_qs    = Payment.objects.filter(created_at__date__gte=start_date, created_at__date__lte=end_date)
    paid_payments  = payments_qs.filter(payment_status='PAID')
    failed_payments = payments_qs.filter(payment_status='FAILED')
    refunded_amount = payments_qs.filter(payment_status__in=['REFUNDED', 'PARTIALLY_REFUNDED']).aggregate(s=Sum('amount'))['s'] or Decimal('0')
    ...
    wallet_txns_qs = WalletTransaction.objects.filter(created_at__date__gte=start_date, created_at__date__lte=end_date)
    ...
    total_refund_amount = Return.objects.filter(status='REFUNDED').aggregate(s=Sum('refund_amount'))['s'] or Decimal('0')

    total_carts      = Cart.objects.count()
    cart_items_total = CartItem.objects.aggregate(s=Sum('quantity'))['s'] or 0
    total_wishlists  = Wishlist.objects.count()

    top_products = items_qs.values('variant__product__product_name').annotate(total=Sum('quantity'), revenue=Sum('total_price')).order_by('-total')[:10]
    top_categories = items_qs.values('variant__product__subcategory__category__category_name').annotate(total=Sum('quantity'), revenue=Sum('total_price')).order_by('-total')[:10]
    top_brands = items_qs.values('variant__product__brand').annotate(total=Sum('quantity'), revenue=Sum('total_price')).order_by('-total')[:10]
    top_subcategories = items_qs.values('variant__product__subcategory__subcategory_name').annotate(total=Sum('quantity'), revenue=Sum('total_price')).order_by('-total')[:5]
    top_colors = items_qs.values('variant__color__color').annotate(total=Sum('quantity')).order_by('-total')[:5]
    top_age_groups = items_qs.values('variant__age_group__age').annotate(total=Sum('quantity')).order_by('-total')[:5]
    ...
    for i in range(num_points - 1, -1, -1):
        ...
        daily_revenue.append(float(day_orders.aggregate(s=Sum('final_amount'))['s'] or 0))
        products_per_day.append(int(OrderItem.objects.filter(order__in=day_orders).aggregate(s=Sum('quantity'))['s'] or 0))
    ...
    for m in range(11, -1, -1):
        ...
        mo_end = (ref.replace(month=ref.month + 1, day=1) - timedelta(days=1)) if ref.month != 12 else ref.replace(month=12, day=31)
        rev = Order.objects.filter(payment_status='PAID', order_date__date__gte=mo_start, order_date__date__lte=mo_end).aggregate(s=Sum('final_amount'))['s'] or 0
        monthly_labels.append(ref.strftime("%b %Y"))
        monthly_revenue.append(float(rev))

    status_data = dict(orders_qs.values('order_status').annotate(count=Count('id')).values_list('order_status', 'count'))
    payment_method_data = dict(paid_orders.values('payment_method').annotate(count=Count('id')).values_list('payment_method', 'count'))
    gender_data = dict(items_qs.values('variant__product__gender').annotate(total=Sum('quantity')).values_list('variant__product__gender', 'total'))

    recent_orders  = orders_qs.select_related('user', 'address').order_by('-order_date')[:20]
    low_stock_items = Inventory.objects.filter(quantity_available__lte=5).select_related('variant__product', 'variant__color', 'variant__age_group').order_by('quantity_available')[:10]

    context = {
        'start_date': start_date.strftime('%Y-%m-%d'),
        'end_date':   end_date.strftime('%Y-%m-%d'),
        'total_orders': total_orders, 'completed_orders': completed_orders,
        'cancelled_orders': cancelled_orders, 'pending_orders': pending_orders,
        'shipped_orders': shipped_orders, 'returned_orders': returned_orders,
        'completion_rate': completion_rate, 'cancellation_rate': cancellation_rate,
        'order_growth': order_growth,
        'total_revenue': total_revenue, 'prev_revenue': prev_revenue,
        'revenue_growth': revenue_growth, 'avg_order_value': avg_order_value,
        'total_discount': total_discount, 'coupon_discount': coupon_discount,
        'shipping_revenue': shipping_revenue,
        'products_sold': products_sold, 'total_products': total_products,
        'active_products': active_products, 'inactive_products': inactive_products,
        'total_categories': total_categories, 'total_subcategories': total_subcategories,
        'total_variants': total_variants, 'total_stock': total_stock,
        'total_reserved': total_reserved, 'total_sold_stock': total_sold_stock,
        ...
        'blocked_customers': blocked_customers, 'new_customers': new_customers,
        'customer_growth': customer_growth, 'customer_growth_10days': customer_growth,
        'customer_growth_30days': customer_growth, 'ordering_customers': ordering_customers,
        'repeat_customers': repeat_customers, 'prev_customers': prev_customers,
        'paypal_revenue': paypal_revenue, 'cod_revenue': cod_revenue,
        'wallet_revenue': wallet_revenue, 'refunded_amount': refunded_amount,
        'failed_payments': failed_payments.count(),
        'total_wallet_balance': total_wallet_balance, 'wallet_credits': wallet_credits,
        'wallet_debits': wallet_debits, 'wallet_refunds': wallet_refunds,
        'total_coupons': total_coupons, 'active_coupons': active_coupons,
        'coupon_uses': coupon_uses, 'total_offers': total_offers,
        'active_offers': active_offers, 'total_referrals': total_referrals,
        'referral_uses': referral_uses,
        'total_reviews': total_reviews, 'approved_reviews': approved_reviews,
        'avg_rating': round(float(avg_rating), 2),
        'total_returns': total_returns, 'approved_returns': approved_returns,
        'pending_returns': pending_returns, 'refunded_returns': refunded_returns,
        'total_refund_amount': total_refund_amount,
        'total_carts': total_carts, 'cart_items_total': cart_items_total,
        'total_wishlists': total_wishlists,
        'top_products': top_products, 'top_categories': top_categories,
        'top_brands': top_brands, 'top_subcategories': top_subcategories,
        'top_colors': top_colors, 'top_age_groups': top_age_groups,
        'recent_orders': recent_orders, 'low_stock_items': low_stock_items,
...
def download_dashboard_pdf(request):
    ...
    top_products = list(items_qs.values('variant__product__product_name').annotate(total=Sum('quantity'), rev=Sum('total_price')).order_by('-total')[:10])
    top_categories = list(items_qs.values('variant__product__subcategory__category__category_name').annotate(total=Sum('quantity'), rev=Sum('total_price')).order_by('-total')[:10])
    top_brands = list(items_qs.values('variant__product__brand').annotate(total=Sum('quantity'), rev=Sum('total_price')).order_by('-total')[:10])
    recent_orders = list(orders_qs.select_related('user').order_by('-order_date')[:15])
    low_stock_items = list(Inventory.objects.filter(quantity_available__lte=5).select_related('variant__product', 'variant__color', 'variant__age_group').order_by('quantity_available')[:10])
    ...
    _build_pdf_header(elements, font_regular, font_bold,
    ...
    elements.append(_section_heading("Product & Inventory Analytics", font_bold, "#0f766e"))
    prod_kpis = [
    ...
    # ── PAYMENT & WALLET KPIs ──
    elements.append(_section_heading("Payment & Wallet Analytics", font_bold, "#92400e"))
    pay_kpis = [
    ...
    elements.append(_section_heading("Coupons, Offers & Referrals", font_bold, "#be185d"))
    misc_kpis = [
    ...
    if top_products:
        elements.append(_data_table(
            ...
            [(i + 1, p['variant__product__product_name'], p['total'], f"₹{p['rev']:,.2f}") for i, p in enumerate(top_products)],
            font_regular, font_bold,
            col_widths=[1 * cm, 10 * cm, 3.5 * cm, 4.5 * cm]
        ))
    else:
        elements.append(Paragraph("No product data for this period.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if top_categories:
        elements.append(_data_table(
            ...
            [(i + 1, c['variant__product__subcategory__category__category_name'], c['total'], f"₹{c['rev']:,.2f}") for i, c in enumerate(top_categories)],
            font_regular, font_bold,
            col_widths=[1 * cm, 10 * cm, 3.5 * cm, 4.5 * cm]
        ))
    else:
        elements.append(Paragraph("No category data for this period.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if top_brands:
        elements.append(_data_table(
            ...
            [(i + 1, b['variant__product__brand'], b['total'], f"₹{b['rev']:,.2f}") for i, b in enumerate(top_brands)],
            font_regular, font_bold,
            col_widths=[1 * cm, 10 * cm, 3.5 * cm, 4.5 * cm]
        ))
    else:
        elements.append(Paragraph("No brand data for this period.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if recent_orders:
        elements.append(_data_table(
            ...
            font_regular, font_bold,
            col_widths=[0.7 * cm, 3.5 * cm, 3.8 * cm, 2.5 * cm, 2.5 * cm, 2 * cm, 3 * cm]
        ))
    else:
        elements.append(Paragraph("No orders in this period.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if low_stock_items:
        elements.append(_data_table(
            ...
            [(inv.variant.product.product_name, inv.variant.color.color,
              inv.variant.age_group.age, inv.quantity_available, inv.quantity_reserved)
             for inv in low_stock_items],
            font_regular, font_bold,
            col_widths=[6 * cm, 3 * cm, 3.5 * cm, 3 * cm, 3.5 * cm]
        ))
    else:
        elements.append(Paragraph("No low-stock items.", ParagraphStyle("n", fontName=font_regular)))

    # ── Footer ──
    elements.append(Spacer(1, 20))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#e8ecf0")))
    elements.append(Spacer(1, 6))
...
def admin_sales_report(request):
    ...
    if report_type == 'daily':
        ...
        orders_qs = orders_qs.filter(order_date__date__gte=week_start, order_date__date__lte=end_date)
    ...
    refund_qs           = Return.objects.filter(created_at__date__gte=start_date, created_at__date__lte=end_date)
    ...
    total_refund_amount = refund_qs.filter(status='REFUNDED').aggregate(s=Sum('refund_amount'))['s'] or Decimal('0')
    return_rate         = safe_pct(total_returns, items_sold)

    top_products_sales   = items_qs.values('variant__product__product_name').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10]
    top_categories_sales = items_qs.values('variant__product__subcategory__category__category_name').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10]
    top_brands_sales     = items_qs.values('variant__product__brand').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10]
    ...
    for i in range(num_points - 1, -1, -1):
        ...
        daily_labels.append(d.strftime("%d %b"))
        day_orders = orders_qs.filter(order_date__date=d)
        daily_revenue_list.append(float(day_orders.aggregate(s=Sum('final_amount'))['s'] or 0))
        daily_orders_list.append(day_orders.count())
        daily_items_list.append(int(items_qs.filter(order__in=day_orders).aggregate(s=Sum('quantity'))['s'] or 0))

    all_orders_in_range = Order.objects.filter(order_date__date__gte=start_date, order_date__date__lte=end_date)
    status_breakdown = dict(all_orders_in_range.values('order_status').annotate(c=Count('id')).values_list('order_status', 'c'))
    payment_breakdown = dict(orders_qs.values('payment_method').annotate(c=Count('id')).values_list('payment_method', 'c'))

    context = {
        'start_date': start_date.strftime('%Y-%m-%d'),
        'end_date':   end_date.strftime('%Y-%m-%d'),
        'report_type': report_type,
        'total_sales': total_sales, 'gross_sales': gross_sales, 'net_sales': net_sales,
        'total_orders': total_orders_count, 'items_sold': items_sold,
        'total_discount': total_discount, 'coupon_discount': coupon_discount,
        'shipping_collected': shipping_collected, 'avg_order_value': avg_order_value,
        'max_order_value': max_order_value, 'min_order_value': min_order_value,
        'free_shipping_orders': free_shipping_orders, 'paid_shipping_orders': paid_shipping_orders,
        'coupon_orders': coupon_orders,
        'total_returns': total_returns, 'approved_returns': approved_returns,
        'refunded_returns': refunded_returns, 'total_refund_amount': total_refund_amount,
        'return_rate': return_rate,
        'paypal_sales': paypal_sales, 'cod_sales': cod_sales, 'wallet_sales': wallet_sales,
        'paypal_count': paypal_count, 'cod_count': cod_count, 'wallet_count': wallet_count,
...
def download_sales_report_pdf(request):
    ...
    top_products_sales   = list(items_qs.values('variant__product__product_name').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10])
    top_categories_sales = list(items_qs.values('variant__product__subcategory__category__category_name').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10])
    top_brands_sales     = list(items_qs.values('variant__product__brand').annotate(qty=Sum('quantity'), rev=Sum('total_price')).order_by('-rev')[:10])
    ...
    _build_pdf_header(elements, font_regular, font_bold,"Sales Report",f" — {rtype_label}",fs, fe)
    ...
    elements.append(_data_table(
        ...
        [
            ("PayPal", paypal_count, f"₹{paypal_sales:,.2f}", f"{safe_pct(float(paypal_sales), float(total_sales))}%"),
            ("Cash on Delivery", cod_count, f"₹{cod_sales:,.2f}", f"{safe_pct(float(cod_sales), float(total_sales))}%"),
            ("Wallet", wallet_count, f"₹{wallet_sales:,.2f}", f"{safe_pct(float(wallet_sales), float(total_sales))}%"),
    ...
    elements.append(_data_table(
        ...
        [
            ...
            ("Approved Returns", str(approved_returns)),
            ("Refunded Returns", str(refunded_returns)),
            ("Pending Returns", str(total_returns - approved_returns - refunded_returns)),
    ...
    if top_products_sales:
        elements.append(_data_table(
            ...
            [(i + 1, p['variant__product__product_name'], p['qty'], f"₹{p['rev']:,.2f}") for i, p in enumerate(top_products_sales)],
            font_regular, font_bold,
            col_widths=[1 * cm, 10 * cm, 3.5 * cm, 4.5 * cm]
        ))
    else:
        elements.append(Paragraph("No product data.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if top_categories_sales:
        elements.append(_data_table(
            ["#", "Category", "Qty Sold", "Revenue (₹)"],
            [(i + 1, c['variant__product__subcategory__category__category_name'], c['qty'], f"₹{c['rev']:,.2f}") for i, c in enumerate(top_categories_sales)],
            font_regular, font_bold,
            col_widths=[1 * cm, 10 * cm, 3.5 * cm, 4.5 * cm]
        ))
    else:
        elements.append(Paragraph("No category data.", ParagraphStyle("n", fontName=font_regular)))
    elements.append(Spacer(1, 10))
    ...
    if all_orders:
        elements.append(_data_table(
            ["#", "Order ID", "Customer", "Date", "Gross(₹)", "Disc(₹)", "Ship(₹)", "Final(₹)", "Method", "Status"],
            [(i + 1, o.order_id,
            ...
            font_regular, font_bold,
            col_widths=[0.6*cm, 3.2*cm, 3*cm, 2.3*cm, 1.8*cm, 1.5*cm, 1.5*cm, 2.2*cm, 1.8*cm, 2.1*cm]
        ))
    else:
        elements.append(Paragraph("No orders in this period.", ParagraphStyle("n", fontName=font_regular)))
